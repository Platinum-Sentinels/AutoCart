...
def parse_price(price):
    if isinstance(price, str):
        # Remove non-numeric characters except the last period
        price = re.sub(r"[^\d.]", "", price)  
        
        # Handle cases with multiple periods by keeping only the last one
        if price.count('.') > 1:
            parts = price.split('.')
...
if __name__ == "__main__":
    ...
    search_query = "i phone 15"  # Example search
    max_pages = 1
